(function () {
    var PROGRESS_RENDER_EPSILON_SECONDS = 0.03;
    var PROGRESS_SEEK_RESET_THRESHOLD_SECONDS = 1.5;

    var sampledProgressSeconds = 0;
    var sampledDurationSeconds = 0;
    var sampledAtMilliseconds = 0;
    var lastRenderedProgressSeconds = null;
    ...
    function show(options) {
        ...
        if (Object.prototype.hasOwnProperty.call(config, 'playbackState')) {
            var playbackState = Number(config.playbackState);
    ...
    function formatTimeInMinutesAndSeconds(seconds) {
        ...
        var secondsPart = Math.floor(seconds % 60).toString().padStart(2, '0');
    ...
    function getProjectedProgressSeconds(nowMilliseconds) {
        var projectedProgressSeconds = Number(sampledProgressSeconds || 0);
        var durationSeconds = Number(sampledDurationSeconds || 0);
    ...
    function renderCurrentProgress(forceRender) {
        ...
        var projectedProgressSeconds = getProjectedProgressSeconds();
    ...
    function incomingSampleRequiresSnapToPosition(samplePositionSeconds, projectedBeforeSampleSeconds, shouldPauseClock) {
        var driftFromProjectedSeconds = samplePositionSeconds - projectedBeforeSampleSeconds;
    ...
    function applyProgressSample(progressPositionSeconds, progressDurationSeconds, options) {
        var nextDurationSeconds = Number(progressDurationSeconds || 0);
        var samplePositionSeconds = Number(progressPositionSeconds || 0);
        var nowMilliseconds = getCurrentHighResolutionTimeMilliseconds();
        var projectedBeforeSampleSeconds = getProjectedProgressSeconds(nowMilliseconds);
    ...
    function consumePlaybackSnapshot(snapshot) {
        ...
        var durationSeconds = Number(snapshot.duration || 0);
        var positionSeconds = Number(snapshot.position || 0);
        var isPlaying = Number(snapshot.state) === MediaPlaybackState.Playing;
